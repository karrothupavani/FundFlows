...
import { Form, Table, Button, Input, Space, Select, DatePicker, InputNumber,Popconfirm,message } from 'antd';
...
const MyTable = () => {
  ...
  const idTokenData =()=>{
    ...
    try {
      axios
        .get(`https://api.p360.build:9003/v1/user/fetchUserDetails/${mail}`, {
          headers: {
...
useEffect(() => {
 ...
 getData();
  ...
  // const filterOptions = {
  //   transactionType: ['BLR', 'CO', 'HYD', 'RAN'], // Add more options as needed
  //   status: ['Draft', 'submitted'], // Add more options as needed
  //   ledger: [
  ...
  //   ],
  //   // Add filter options for other dropdown fields here
  // };
  const columns = [
    {
      title: 'Type',
    ...
    },
    {
      ...
      filters: [
        { text: 'Advance Tax 1%', value: 'Advance Tax 1%' },
        { text: 'Admin Charges 4%', value: 'Admin Charges 4%' },
        { text: 'Ad charges 1% B2B', value: 'Ad charges 1% B2B' },
      ],
  ...
  const renderLedgerCell = (record, dataIndex, text) => {
    ...
    if (isEditing) {
      return (
        ...
        >
          <Option value="Advance Tax 1%">Advance Tax 1%</Option>
          <Option value="Admin Charges 4%">Admin Charges 4%</Option>
          <Option value="Ad charges 1% B2B">Ad charges 1% B2B</Option>
        </Select>
  ...
  const getData = ()=>{
    try {
      axios
        .get('https://api.p360.build:6060/v1/fundflow/colevis/getCoLevies', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    try {
      const response = await axios.post(
        'https://api.p360.build:6060/v1/fundflow/colevis/addCoLevies',
        {
          ...
          documentData: newRowData.documentDate,
  ...
  const handleSaveEdit = async (record) => {
    if (editedData[record.key]) {
      ...
      if (editedRowData.status === "submitted") {
        ...
        editedRowData.submittedBy = userName;
    ...
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ padding: '25px', width: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
          <Typography variant="h4">CO Levies</Typography>
         <div>
          <Button
        type="primary"
