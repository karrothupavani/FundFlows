...
import { EditOutlined, SaveOutlined, CloseOutlined ,CopyOutlined,HomeOutlined} from '@ant-design/icons';
...
useEffect(() => {
  ...
  try {
    axios
      .get('https://api.p360.build:6060/v1/fundflow/termloansetoff/fetchAll', {
        headers: {
...
const handleDuplicate = (record) => {
  ...
  const calculateYearAndPeriod = (documentDate) => {
    ...
    const period = date.month() >= 3 ? date.month() - 2 : date.month() + 10; // Adjusting for April to March fiscal year
    return { year, period };
  };
    ...
    if (isEditing) {
      return (
        <DatePicker
          style={{ width: '100%' }}
          format="DD-MM-YYYY"
          value={editedData[record.key]?.[dataIndex] ? moment(editedData[record.key][dataIndex], 'DD-MM-YYYY') : null ||editedData[record.key]?.[dataIndex]}
  ...
  const handleEditInputChange = (value, record, field) => {
    if (field === 'documentDate') {
      ...
      // Update the editedData state with the new values
      setEditedData((prevEditedData) => ({
        ...prevEditedData,
        [record.key]: {
          ...(prevEditedData[record.key] || {}),
          documentDate: value, // Update documentDate
          year, // Update year
    ...
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ padding: '25px', width: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
          <Typography variant="h4">Term Loan SetOff</Typography>
         <div>
          <Button
        type="primary"
